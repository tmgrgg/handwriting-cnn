...
public class WhitespaceSegmenter extends Segmenter {
	
	public WhitespaceSegmenter(Preprocessor preprocessor) {
		super(preprocessor);
	}
		...
		for (int x = 0; x < workingPixelData[0].length; x++) {
			for (int y = 0; y < workingPixelData.length; y++) {
				if (workingPixelData[y][x] != ImageProcessLibrary.WHITE) {
					...
					segments.add(new Segment(ImageProcessLibrary
							.imageFromPixelData(newPixelData), top, bottom,
							right, left));
				}
			}
		}
	...
	private void getSegmentParameters(int x, int y) {
		...
		if (workingPixelData[y][x] != -1) {
			workingPixelData[y][x] = -1;
