 ...
 *       
 *       
 *       Best solution currently is to either use horizontal histogram minima which is... poor because it doesn't allow 
 *       for slanted lines...
 *       
...
public class LineParserForDisconnectedComponents implements Parser {
	...
	private void findLines(TranscriptionData data) {
		...
		while (!data.segments.isEmpty()) {
			...
			/*
			 * 3. Find the first (left-most) segment whose / midpoint lies
			 * within the top and bottom boundaries of the previous letter.
			 */

			// Currently only consideirng bottomBound because in theory we are
			// always working on the top line (that's why we use the bottom)
			int topBound = current.getTop();
			int bottomBound = current.getBottom();
			for (Segment segment : data.segments) {

				int midpoint = (int) Math.round(1*segment.getTop() + 0*segment.getBottom());
				if ((segment.getLeft() >= current.getLeft())
						&& (midpoint <= bottomBound)) {
					if ((segment.getBottom() - segment.getTop()) >= HEIGHT_THRESHOLD
							* (bottomBound - topBound)) {
						topBound = segment.getTop();
						bottomBound = segment.getBottom();
					}
					current = segment;
					changed = true;
					break;
