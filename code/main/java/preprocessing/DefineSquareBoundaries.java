...
/*USAGE:
 * 
 * Difference between this and normal DefineBoundaries is that we enforce that the whitespace background forms a perfect square
 * 
...
public class DefineSquareBoundaries implements ImageProcess {
	...
	public int[][] process(int[][] pixelData) {
		...
		int widthOffset = (size - width)/2;
		int heightOffset = (size - height)/2;
		

		//guaranteed to be all-zero
		int[][] newPixelData = new int[size][size];
